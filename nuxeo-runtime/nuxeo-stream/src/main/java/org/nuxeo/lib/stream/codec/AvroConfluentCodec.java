...
/**
 * Use the Confluent Avro encoding which differs from Avro message,
 * the schema store is a REST Confluent Schema Store.
 *
 * @since 10.3
 */
public class AvroConfluentCodec<T> implements Codec<T> {
    ...
    public AvroConfluentCodec(Class<T> messageClass, String baseUrl) {
        this.messageClass = messageClass;
        schema = ReflectData.get().getSchema(messageClass);
        schemaName = messageClass.getName();
        client = new CachedSchemaRegistryClient(baseUrl, 5);
        try {
    ...
    public T decode(byte[] data) {
        ...
        RawMessageDecoder<T> decoder = new RawMessageDecoder<>(ReflectData.get(), schema);
        try {
